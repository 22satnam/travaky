...
// //         )}
// //         {sectionIndex === 6 && (
// //           <PaymentSection data={traveler} travelerCount={totalTravelers}  onPayment={openPayment} />
// //         )}
// //       </div>
...
//   travelerIndex: number
//   totalTravelers: number
//   openPayment?: () => Promise<void> // ✅ New optional prop
// }
...
export default function SectionCard({
  ...
  useEffect(() => {
    ...
    if (sectionIndex === 0) {
      ...
      setIsValid(has('appointmentDate') && has('appointmentAddress') && has('appointmentContact'))
    } else {
      setIsValid(true)
    }
  }, [traveler, sectionIndex])
  ...
  return (
    <div className="rounded-2xl border bg-background shadow-lg p-5 sm:p-6 lg:p-8 space-y-6">
      <div className="flex justify-between items-center">
        <h2 className="text-xl font-semibold">{sectionTitles[sectionIndex]}</h2>
        {sectionIndex <= 3 && (
          <p className="text-sm text-muted-foreground">
