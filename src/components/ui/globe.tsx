...
// type Label = { lat: number; lng: number; text: string; size?: number; color?: string };
...
//     // —— NEW: labels (country names) ——
//     if (labels && labels.length) {
//       globeRef.current
//         .labelsData(labels)
//         .labelText((d: any) => d.text)
//         .labelSize((d: any) => d.size ?? 1.2)
//         .labelColor((d: any) => d.color ?? "#e2e8f0")
//         .labelResolution(2)
//         .labelAltitude(0.01)
//         .labelDotRadius(0.2);
//     } else {
//       globeRef.current.labelsData([]); // clear
//     }
//   }, [
//     isInitialized,
//     data,
...
//         .map((d: any) => ({ lat: d.startLat, lng: d.startLng, color: d.color }));
...
export function Globe({ globeConfig, data }: WorldProps) {
  ...
  useEffect(() => {
    ...
    globeRef.current
      ...
      .arcAltitude((e)  => (e as any).arcAlt * 1)
      .arcStroke(() => [0.32, 0.28, 0.3][Math.round(Math.random() * 2)])
      .arcDashLength(defaultProps.arcLength)
    ...
    // include defaults that affect rebuild
  ]);
...
// NEW: keep camera aspect in sync with canvas size
function CameraRig() {
  const { camera, size } = useThree();
  useEffect(() => {
    camera.position.set(0, 0, cameraZ);
    // @ts-ignore
    camera.near = 180; // ensure same near/far as before
    // @ts-ignore
    camera.far = 1800;
