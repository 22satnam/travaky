...
export async function POST(req: Request) {
  try {
    ...
    await supabase.from('auth_whatsapp_otp').update({ used: true }).eq('id', row.id);
    ...
    return NextResponse.json({ ok: false, error: e?.message ?? 'WhatsApp OTP verify failed' }, { status: 400 });
  }
}
