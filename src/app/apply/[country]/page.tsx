...
export default function ApplyFormPage() {
  ...
  const submitToBackend = async () => {
    ...
    try {
      ...
      if (result.success && result.sessionId && result.pdfUrl) {
        setSubmitted(true)
        localStorage.setItem('sessionId', result.sessionId)
        localStorage.setItem('pdfUrl', result.pdfUrl)

        openPayment(payload.selectedPlan, result.sessionId, payload.country, payload.email, payload.phone, result.pdfUrl)
      } else {
        throw new Error('PDF or Session ID generation failed')
      }
    } catch (err) {
      console.error('Submission failed:', err)
  ...
  async function openPayment(plan: string, sessionId: string, country: string, email: string, phone: string, pdfUrl: string) {
    const prices = {
      "Docs on Call": 7500,
      "Docs on Door": 9500,
      "Visa at Door": 12500
    ...
    const options = {
      ...
      currency: "INR",
      name: "Travaky",
      description: `${plan} Plan for ${country}`,
      prefill: {
        email: email,
        contact: phone,
      },
      ...
      <div className="grid grid-cols-1 md:grid-cols-5 gap-6 max-w-7xl mx-auto p-6">
        <div className="md:col-span-1">
          <SectionSidebar
            ...
            completedSteps={completedSections}
            onNavigate={setActiveSection}
            isStepDisabled={(step) => step > 3 && !allTravelersDone}
          />
        </div>
